...
/** An unordered map that uses float keys. This implementation is a cuckoo hash map using 3 hashes, random walking, and a small stash
 ...
 * @author Maik Macho (Ported IntMap to FloatMap)
 * @author Nathan Sweet */
public class FloatMap<V> {
	...
	public FloatMap(int initialCapacity, float loadFactor) {
		...
		keyTable = new float[capacity + stashCapacity];
		valueTable = (V[])new Object[keyTable.length];
	}

	public V put (int key, V value) {
		return put((float)key, value);
	}
	...
	public V put (float key, V value) {
		...
		float[] keyTable = this.keyTable;

		// Check for existing keys.
		int index1 = Float.floatToIntBits(key) & mask;
		float key1 = keyTable[index1];
		if (key1 == key) {
	...
	public void putAll (FloatMap<V> map) {
		for (Entry<V> entry : map.entries())
			put(entry.key, entry.value);
	}

	/** Skips checks for existing keys. */
	private void putResize (float key, V value) {
		if (key == 0) {
			zeroValue = value;
			hasZeroValue = true;
			return;
	...
	private void push (float insertKey, V insertValue, int index1, float key1, int index2, float key2, int index3, float key3) {
		float[] keyTable = this.keyTable;
		...
		do {
			...
			index1 = Float.floatToIntBits(evictedKey) & mask;
	...
	private void putStash (float key, V value) {
	...
	public V get (float key) {
		...
		int index = Float.floatToIntBits(key) & mask;
	...
	private V getStash (float key, V defaultValue) {
		float[] keyTable = this.keyTable;
		for (int i = capacity, n = i + stashSize; i < n; i++)
	...
	private boolean containsKeyStash (float key) {
		float[] keyTable = this.keyTable;
		for (int i = capacity, n = i + stashSize; i < n; i++)
	 ...
	 * @param identity If true, uses == to compare the specified value with values in the map. If false, uses
	 *           {@link #equals(Object)}. */
	public float findKey (Object value, boolean identity, float notFound) {
		V[] valueTable = this.valueTable;
		if (value == null) {
	...
	private void resize (int newSize) {
		...
		float[] oldKeyTable = keyTable;
		V[] oldValueTable = valueTable;

		keyTable = new float[newSize + stashCapacity];
		valueTable = (V[])new Object[newSize + stashCapacity];

		size = hasZeroValue ? 1 : 0;
		stashSize = 0;
		for (int i = 0; i < oldEndIndex; i++) {
			float key = oldKeyTable[i];
			if (key != EMPTY) putResize(key, oldValueTable[i]);
		}
	}

	private int hash2 (float h) {
        int hh = Float.floatToIntBits(h);
        hh *= PRIME2;
		return (int)((hh ^ hh >>> hashShift) & mask);
	}

	private int hash3 (float h) {
        int hh = Float.floatToIntBits(h);
        hh *= PRIME3;
		return (int)((hh ^ hh >>> hashShift) & mask);
	}
	...
	static private class MapIterator<V> {
		...
		final FloatMap<V> map;
		int nextIndex, currentIndex;
		boolean valid = true;

		public MapIterator (FloatMap<V> map) {
			this.map = map;
			reset();
		}
	...
	static public class Keys extends MapIterator {
		...
		public float next () {
			if (!hasNext) throw new NoSuchElementException();
			if (!valid) throw new GdxRuntimeException("#iterator() cannot be used nested.");
			float key = nextIndex == INDEX_ZERO ? 0 : map.keyTable[nextIndex];
			currentIndex = nextIndex;
			findNextIndex();
			return key;
		}
