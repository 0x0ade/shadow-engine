...
/** An unordered map that uses byte keys. This implementation is a cuckoo hash map using 3 hashes, random walking, and a small stash
 ...
 * @author Maik Macho (Ported IntMap to ByteMap)
 * @author Nathan Sweet */
public class ByteMap<V> {
	 ...
	 * table. */
	public ByteMap (int initialCapacity) {
		this(initialCapacity, 0.8f);
	}

	/** Creates a new map with the specified initial capacity and load factor. This map will hold initialCapacity * loadFactor items
	 * before growing the backing table. */
	public ByteMap (int initialCapacity, float loadFactor) {
		...
		keyTable = new byte[capacity + stashCapacity];
		valueTable = (V[])new Object[keyTable.length];
	}
	
	public V put (int key, V value) {
		return put((byte)key, value);
	}
	...
	public V put (byte key, V value) {
		...
		byte[] keyTable = this.keyTable;

		// Check for existing keys.
		int index1 = key & mask;
		byte key1 = keyTable[index1];
		if (key1 == key) {
	...
	private void putResize (byte key, V value) {
		if (key == 0) {
			zeroValue = value;
			hasZeroValue = true;
			return;
	...
	private void push (byte insertKey, V insertValue, int index1, byte key1, int index2, byte key2, int index3, byte key3) {
		byte[] keyTable = this.keyTable;
	...
	private void putStash (byte key, V value) {
	...
	public V get (byte key, V defaultValue) {
		if (key == 0) {
	...
	private V getStash (byte key, V defaultValue) {
		byte[] keyTable = this.keyTable;
		for (int i = capacity, n = i + stashSize; i < n; i++)
	...
	private boolean containsKeyStash (byte key) {
		byte[] keyTable = this.keyTable;
		for (int i = capacity, n = i + stashSize; i < n; i++)
	...
	private void resize (int newSize) {
		...
		byte[] oldKeyTable = keyTable;
		V[] oldValueTable = valueTable;

		keyTable = new byte[newSize + stashCapacity];
		valueTable = (V[])new Object[newSize + stashCapacity];

		size = hasZeroValue ? 1 : 0;
		stashSize = 0;
		for (int i = 0; i < oldEndIndex; i++) {
			byte key = oldKeyTable[i];
			if (key != EMPTY) putResize(key, oldValueTable[i]);
		}
	}

	private int hash2 (byte h) {
		h *= PRIME2;
		return (int)((h ^ h >>> hashShift) & mask);
	}

	private int hash3 (byte h) {
		h *= PRIME3;
		return (int)((h ^ h >>> hashShift) & mask);
	}
	...
	static private class MapIterator<V> {
		...
		public MapIterator (ByteMap<V> map) {
			this.map = map;
			reset();
		}
	...
	static public class Keys extends MapIterator {
		...
		public byte next () {
			if (!hasNext) throw new NoSuchElementException();
			if (!valid) throw new GdxRuntimeException("#iterator() cannot be used nested.");
			byte key = nextIndex == INDEX_ZERO ? 0 : map.keyTable[nextIndex];
			currentIndex = nextIndex;
			findNextIndex();
			return key;
		}
		...
		public ByteArray toArray () {
			ByteArray array = new ByteArray(true, map.size);
			while (hasNext)
				array.add(next());
			return array;
		}
